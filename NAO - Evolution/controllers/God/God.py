...
class God:
    ...
    def create_mutated_individual(self, start_filename, i):
        ...
        percentuage_to_change = range(8, 11 + 1)  # [from, to] 5% - 10% # need the +1
        min_change = 0.15  # 0.1, not 1% !
...
class Driver (Supervisor):
    ...
    def run(self):
        ...
        print("simulating: " + str(motion_file_evaluated) +"                 GENERATION -> " + str(self.god.n_generation)+"   MAX_SCORE -> " + str(max(self.god.scores)) + "(ind "+ str(self.god.scores.index(max(self.god.scores)))  +" )")
...
# util.add_initial_time_to_motion_file(0.1) # you need to save it after with saveCopy()
# util.saveCopy()
#
# util = motion_util()
